//! Prime Storage - Storage backends and persistence
...
impl StorageBackend for MmapStorage {
    fn save(&mut self, graph: &KnowledgeGraph) -> Result<()> {
        // Save using binary storage, then remap
        let mut binary = BinaryStorage::new(StorageConfig {
            path: self.path.clone(),
            ..self.config.clone()
        });
...
impl StorageManager {
    ...
    pub fn with_composite(config: StorageConfig) -> Self {
        ...
        let composite = CompositeStorage::new(primary).with_secondary(Box::new(BinaryStorage::new(config.clone())));
